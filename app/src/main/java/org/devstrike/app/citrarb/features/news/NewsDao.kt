...
interface NewsDao {
    ...
    @Query("SELECT * FROM LocalNewsList WHERE locallyDeleted =0 ORDER BY savedDate DESC")
    fun getAllNewsListOrderByDate(): Flow<List<LocalNewsList>>

    //function to actually delete the local note
    @Query("DELETE FROM LocalNewsList WHERE uid=:newsItemId")
    ...
    @Query("UPDATE LocalNewsList SET locallyDeleted = 1 WHERE uid = :newsItemId")
