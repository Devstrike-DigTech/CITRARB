...
@AndroidEntryPoint
class TechConnect: BaseFragment<ConnectViewModel, FragmentConnectOccupationLayoutBinding, ConnectRepoImpl>() {
    ...
    private fun subscribeToAllConnectEvent() {
        lifecycleScope.launch {
            ...
            connectViewModel.savedConnectsList.collect { result ->
                ...
                val techConnect = mutableListOf<Connect>()
                for (connect in result){
                    if (connect.category == "Tech"){
                        techConnect.add(connect)
                    }
                }
                if (result.isEmpty()){
                    requireContext().toast("No Connects")
                }else{
                    connectAdapter.connects = techConnect.filter { connect ->
