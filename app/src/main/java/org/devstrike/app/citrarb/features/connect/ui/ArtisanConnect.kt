...
@AndroidEntryPoint
class ArtisanConnect: BaseFragment<ConnectViewModel, FragmentConnectOccupationLayoutBinding, ConnectRepoImpl>() {
    ...
    private fun subscribeToAllConnectEvent() {
        lifecycleScope.launch {
            ...
            connectViewModel.savedConnectsList.collect { result ->
                ...
                val serviceConnect = mutableListOf<Connect>()
                for (connect in result){
                    if (connect.category == "Service"){
                        serviceConnect.add(connect)
                    }
                }
                if (serviceConnect.isEmpty()){
                    requireContext().toast("No Service Connects")
                }else{
                    connectAdapter.connects = serviceConnect.filter { connect ->
