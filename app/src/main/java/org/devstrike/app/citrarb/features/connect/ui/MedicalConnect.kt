 ...
 */
@AndroidEntryPoint
class MedicalConnect: BaseFragment<ConnectViewModel, FragmentConnectOccupationLayoutBinding, ConnectRepoImpl>() {
    ...
    private fun subscribeToAllConnectEvent() {
        lifecycleScope.launch {
            ...
            connectViewModel.savedConnectsList.collect { result ->
                ...
                val medicalConnect = mutableListOf<Connect>()
                for (connect in result){
                    if (connect.category == "Medical"){
                        medicalConnect.add(connect)
                    }
                }
                if (medicalConnect.isEmpty()){
                    requireContext().toast("No Connects")
                }else{
                    connectAdapter.connects = medicalConnect.filter { connect ->
