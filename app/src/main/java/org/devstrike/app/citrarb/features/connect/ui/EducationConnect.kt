...
@AndroidEntryPoint
class EducationConnect: BaseFragment<ConnectViewModel, FragmentConnectOccupationLayoutBinding, ConnectRepoImpl>() {
    ...
    private fun subscribeToAllConnectEvent() {
        lifecycleScope.launch {
            ...
            connectViewModel.savedConnectsList.collect { result ->
                ...
                val educationConnect = mutableListOf<Connect>()
                for (connect in result){
                    if (connect.category == "Education"){
                        educationConnect.add(connect)
                    }
                }
                if (educationConnect.isEmpty()){
                    requireContext().toast("No Connects")
                }else{
                    connectAdapter.connects = educationConnect.filter { connect ->
